...
# Map D0-D4 labels to C/H/O/B/I folder names
label_to_folder = {
...
class SignalPreprocessor:
    """Signal preprocessing pipeline with per-channel min-max normalization."""
    ...
    def build_time_domain_grayscale(self, sig_1d: np.ndarray) -> Image.Image:
        """Convert 1D signal to time-domain grayscale image (single channel)"""
    ...
    def build_log_spectrum_image(self, sig_1d: np.ndarray, fs: int) -> Image.Image:
        """Convert 1D signal to frequency-domain log-spectrum image (single channel)"""
        yf = rfft(sig_1d)
        amp = np.abs(yf)
    ...
    def build_cwt_timefreq_image(self, sig_1d: np.ndarray, fs: int) -> Image.Image:
        """Convert 1D signal to CWT time-frequency image (single channel, primary channel only)"""
...
def read_single_segment(csv_path, subfile_id, seg_idx, seg_per_subfile, skip_rows, read_cols):
    ...
    Global segment index = subfile_id * seg_per_subfile + seg_idx
    Returns all channel signals.
    """
...
def generate_fold_images(cfg):
    ...
    Time/freq domains use all acquisition channels; time-frequency domain uses primary channel only.
    """
    ...
    for fold_idx in range(1, 6):
        ...
        # ========== Step 1: Fit normalization on ALL channels of training set ==========
        preproc = SignalPreprocessor()
        train_signals_by_channel = {ch: [] for ch in range(cfg["read_cols"])}
        ...
        for _, row in train_df.iterrows():
            ...
            # Collect signals for every channel
            for ch in range(cfg["read_cols"]):
        ...
        for idx, row in train_df.iterrows():
            ...
            # --- Time domain: all channels, stacked as multi-channel tensor ---
            time_ch_list = []
            for ch in range(cfg["read_cols"]):
                img_t_ch = preproc.build_time_domain_grayscale(sig_norm[ch])
                time_ch_list.append(np.array(img_t_ch, dtype=np.uint8))
            time_multi = np.stack(time_ch_list, axis=-1)  # shape: [128, 128, read_cols]
            np.save(os.path.join(save_dir, f"sample_{sid:06d}_time.npy"), time_multi)

            # --- Frequency domain: all channels, stacked as multi-channel tensor ---
            freq_ch_list = []
            for ch in range(cfg["read_cols"]):
                img_f_ch = preproc.build_log_spectrum_image(sig_norm[ch], fs=cfg["fs"])
                freq_ch_list.append(np.array(img_f_ch, dtype=np.uint8))
            freq_multi = np.stack(freq_ch_list, axis=-1)  # shape: [128, 128, read_cols]
            np.save(os.path.join(save_dir, f"sample_{sid:06d}_freq.npy"), freq_multi)

            # --- Time-frequency domain: primary channel only, keep PNG ---
            img_tf = preproc.build_cwt_timefreq_image(sig_norm[cfg["primary_ch"]], fs=cfg["fs"])
            img_tf.save(os.path.join(save_dir, f"sample_{sid:06d}_tf.png"))

            if (idx + 1) % 500 == 0:
                print(f"     Train progress: {idx + 1}/{len(train_df)}")

        # ========== Step 3: Generate validation set images ==========
        ...
        for idx, row in val_df.iterrows():
            ...
            for ch in range(cfg["read_cols"]):
                img_t_ch = preproc.build_time_domain_grayscale(sig_norm[ch])
                time_ch_list.append(np.array(img_t_ch, dtype=np.uint8))
            time_multi = np.stack(time_ch_list, axis=-1)
            np.save(os.path.join(save_dir, f"sample_{sid:06d}_time.npy"), time_multi)

            # Frequency domain: all channels
            freq_ch_list = []
            for ch in range(cfg["read_cols"]):
                img_f_ch = preproc.build_log_spectrum_image(sig_norm[ch], fs=cfg["fs"])
                freq_ch_list.append(np.array(img_f_ch, dtype=np.uint8))
            freq_multi = np.stack(freq_ch_list, axis=-1)
            np.save(os.path.join(save_dir, f"sample_{sid:06d}_freq.npy"), freq_multi)

            # Time-frequency domain: primary channel only
            img_tf = preproc.build_cwt_timefreq_image(sig_norm[cfg["primary_ch"]], fs=cfg["fs"])
            img_tf.save(os.path.join(save_dir, f"sample_{sid:06d}_tf.png"))
...
def generate_cond1_full_images(cfg):
    """Generate full Working Condition 1 image set with global normalization.
    Time/freq domains use all acquisition channels; time-frequency domain uses primary channel only.
    """
    df_full = pd.read_csv(cfg["partition_csv"])
    cond1_df = df_full[df_full["fold"] != -1].reset_index(drop=True)
    ...
    # ========== Step 1: Fit global normalization on ALL channels ==========
    preproc = SignalPreprocessor()
    all_signals_by_channel = {ch: [] for ch in range(cfg["read_cols"])}
...
def generate_cond2_images(cfg):
    ...
    # Fit normalization on full Condition 1 training set (all channels)
